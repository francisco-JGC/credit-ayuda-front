...
export default function ReportMontlyPage() {
  const { formValues, handleInputChange } = useForm({ filterDate: new Date().toISOString().split('T')[0] })
  const { credits, isLoading, error, refetch, summary } = useCredit({ filter_type: 'monthly', date: formValues.filterDate })

  useEffect(() => { refetch() }, [formValues.filterDate])

  return (
    <LayuotPage title="Reporte Mensual">
